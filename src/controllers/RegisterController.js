  ...
  try {
    ...
    if (users != null) {
      ...
      res
        .status(201)
        .render("login", {
          errorMessage: "User Registered! Log in to continue",
        });
    }
  } catch (err) {
    console.log(err);
  ...
  try {
    ...
    if (user != null) {
      ...
      return res.render("login", {
        errorMessage: "Account does not exist! Sign-up to continue",
      });
    }
  } catch (err) {
    console.log(err);
  ...
  try {
    if (req.session.user_id) {
      ...
      if (match) {
        ...
        res.render("login", {
          errorMessage: "password Updated Successfully! please Login.",
        });
      }
    } else {
  ...
  try {
    ...
    if (!user) {
      return res.render("forgotPassword", {
        error: "Email is not registered! Please enter valid email.",
      });
    } else {
      ...
      res.render("forgotPassword", {
        error: "Please check your inbox! A reset link has been sent.",
      });
    }
  } catch (err) {
    console.log(err);
