...
const AddCabRequest = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      return res
        .status(401)
        .render("login", { errorMessage: "User not found" });
    }
...
const SearchCab = async (req, res) => {
  ...
  try {
    ...
    const lowerTimeBound =
      userTimeInMinutes - 120 >= 0
        ? userTimeInMinutes - 120
        : 1440 + (userTimeInMinutes - 120);
    ...
    for (i = 0; i < reqs.length; i++) {
      ...
      if (
        userTimeInMinutesreq > lowerTimeBound &&
        userTimeInMinutesreq < upperTimeBound
      ) {
...
const SearchFlight = async (req, res) => {
  ...
  try {
    const reqs = await Request.RequestsFlight.find({
      $and: [{ date: departure_date }, { Flightnumber: flightNumber }],
    });
    const user = await Register.findOne({ _id: req.session.user_id });
